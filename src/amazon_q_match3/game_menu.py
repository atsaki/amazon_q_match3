...
class GameMenu:
    ...
    def __init__(self, screen, highscore_manager):
        ...
        try:
            ...
            self.button_font = pygame.font.SysFont('arial,helvetica,sans-serif', 28)  # Special font for buttons
    ...
    def _handle_main_menu_click(self, mouse_pos) -> Optional[str]:
        """メインメニューのクリック処理"""
        if self.buttons['start'].collidepoint(mouse_pos):
            self.state = MenuState.TIME_SELECT
            return 'time_select'
        elif self.buttons['highscore'].collidepoint(mouse_pos):
    ...
    def _handle_time_select_click(self, mouse_pos) -> Optional[str]:
        """時間選択のクリック処理"""
        if self.buttons['time_30'].collidepoint(mouse_pos):
            self.selected_time = 30
            return 'start_game'
        elif self.buttons['time_60'].collidepoint(mouse_pos):
            self.selected_time = 60
            return 'start_game'
        elif self.buttons['time_180'].collidepoint(mouse_pos):
    ...
    def _handle_highscore_click(self, mouse_pos) -> Optional[str]:
        """ハイスコア画面のクリック処理"""
    ...
    def _handle_game_over_click(self, mouse_pos) -> Optional[str]:
        """ゲームオーバー画面のクリック処理"""
        if self.buttons['play_again'].collidepoint(mouse_pos):
            ...
            self.state = MenuState.MAIN_MENU
            return 'main_menu'
        elif self.buttons['quit_game'].collidepoint(mouse_pos):
            return 'quit'
        return None
    ...
    def _draw_main_menu(self):
        ...
        if best_score > 0:
            best_text = self.small_font.render(f"Best Score: {best_score} ({best_mode})", True, COLORS['YELLOW'])
    ...
    def _draw_time_select(self):
        ...
        for time_limit, label in [(30, '30s'), (60, '1min'), (180, '3min')]:
            ...
            if best_score > 0:
                score_text = self.small_font.render(f"{label}: {best_score}", True, COLORS['LIGHT_GRAY'])
    ...
    def _draw_game_over(self):
        ...
        # Time limit display
        time_text = self.text_font.render(f"Time Limit: {self.selected_time}s", True, COLORS['LIGHT_GRAY'])
        ...
        if best_score > 0:
            best_text = self.small_font.render(f"Mode Best: {best_score}", True, COLORS['LIGHT_GRAY'])
    ...
    def set_game_over(self, score: int, time_limit: int):
        ...
        self.logger.info(f"Game over: Score={score}, Time={time_limit}s, New highscore={self.is_new_highscore}")
