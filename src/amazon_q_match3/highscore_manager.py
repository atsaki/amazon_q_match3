...
class HighScoreManager:
    ...
    def add_score(self, time_limit: int, score: int, player_name: str = "Player") -> bool:
        ...
        # ハイスコアかどうかチェック
        is_highscore = any(entry["score"] == score and entry["date"] == new_entry["date"] 
                          for entry in self.highscores[time_key])
        
        self._save_highscores()
        
        if is_highscore:
            rank = next(i+1 for i, entry in enumerate(self.highscores[time_key]) 
                       if entry["score"] == score and entry["date"] == new_entry["date"])
            self.logger.info(f"New highscore! Rank {rank} with {score} points in {time_limit}s mode")
